...
class ModeratorCommands(commands.Cog):
    ...
    @commands.command(usage="case <case ID>")
    @utils.guild_only()
    async def case(self, ctx, case_id):
        ...
        await utils.embed(ctx, discord.Embed(colour=utils.case_colour(self.bot, case.punishment), timestamp=datetime.utcnow(), title=f"{case.punishment.title()} | Case #{case_id}").add_field(name="User" if not case.user.bot else "Bot", value=f"{case.user} ({case.user.mention})").add_field(name="Moderator", value=case.mod if hasattr(case.mod, "id") else f"??? (Moderator: do `{ctx.bot.config.prefix}reason {case_id}`)").add_field(name="Reason", value=case.reason if case.reason else f"Moderator: please do `{ctx.bot.config.prefix}reason {case_id} <reason>`", inline=False).set_footer(text=f"ID: {case.user.id}"), override_colour=True)

    @commands.command(usage="reason <case ID> <reason>")
    @utils.guild_only()
    @utils.is_mod()
    ...
    @commands.command(aliases=["prune", "nuke"], usage="purge <limit> [@users...]")
    @utils.guild_only()
    ...
    @commands.command(usage="mute <@users...> [reason]")
    @utils.guild_only()
    @commands.bot_has_permissions(manage_roles=True)
    @utils.is_mod()
    async def mute(self, ctx, users: commands.Greedy[discord.Member], *, reason="No reason provided."):
        ...
        for user in users[:5]:
            if not utils.is_higher_or_equal(ctx.bot, ctx.author, user) or role in user.roles:
                ...
                self.bot.cache.logs.add(f"{user.id}:{ctx.guild.id}:Mute")
    ...
    @commands.command(usage="unmute <@users...> [reason]")
    @utils.guild_only()
    @commands.bot_has_permissions(manage_roles=True)
    @utils.is_mod()
    async def unmute(self, ctx, users: commands.Greedy[discord.Member], *, reason="No reason provided."):
        ...
        for user in users[:5]:
            if not utils.is_higher_or_equal(ctx.bot, ctx.author, user) or role not in user.roles:
                ...
                self.bot.cache.logs.add(f"{user.id}:{ctx.guild.id}:Unmute")
    ...
    @commands.command(aliases=["idban"], usage="hackban <user IDs...> [reason]")
    @utils.guild_only()
    @commands.bot_has_permissions(ban_members=True)
    @utils.is_mod()
            ...
            try:
                ...
                if ctx.guild.get_member(user.id) is not None:
                    ...
                    try:
                        ...
                        self.bot.cache.logs.add(f"{user.id}:{ctx.guild.id}:Ban")
    ...
    @commands.command(usage="ban <@users...> [reason]")
    @utils.guild_only()
    @commands.bot_has_permissions(ban_members=True)
    @utils.is_mod()
    ...
    @commands.command(usage="unban <@users...> [reason]")
    @utils.guild_only()
    @commands.bot_has_permissions(ban_members=True)
    @utils.is_mod()
        ...
        for user in users[:5]:
            try:
                ...
                if ctx.guild.get_member(user.id) is not None:
                    ...
                    try:
                        await ctx.guild.fetch_ban(user)
                        self.bot.cache.logs.add(f"{user.id}:{ctx.guild.id}:Unban")
    ...
    @commands.command(usage="kick <@users...> [reason]")
    @utils.guild_only()
    ...
    async def kick(self, ctx, users: commands.Greedy[discord.Member], *, reason="No reason provided."):
        ...
        for user in users[:5]:
            if not utils.is_higher_or_equal(ctx.bot, ctx.author, user):
                ...
                self.bot.cache.logs.add(f"{user.id}:{ctx.guild.id}:Kick")
